...
class InviteUser extends Component<InviteUserProps, InviteUserState> {
  ...
  onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    //   currentSearchInputValue = this.searchInput.current.
    // } else {
    //   currentSearchInputValue = "";
    // }
    this.doSearchUsers(this.state.userSearch);
  }
  ...
  doSearchUsers(value: string) {
    ...
    if (
      this.lastSearchAt === null ||
      new Date().getTime() - this.lastSearchAt.getTime() >
        InviteUser.searchDebounce
    ) {
  ...
  renderUsers() {
    if (!this.props.users) {
      const message =
        "Search for users by name or username to display a list of available users in this space.";
      return <Alert type="info" message={message} showIcon />;
    }
    if (this.props.users.length === 0) {
      return <Alert type="warning" message="No users found" showIcon />;
    }
    ...
    return this.props.users.map((user) => {
      ...
      switch (user.relation) {
        ...
        case orgModel.UserRelationToOrganization.MEMBER_REQUEST_PENDING:
          ...
          statusTooltip =
            "This user has already requested membership to this Organization (check your requests inbox)";
          break;
        case orgModel.UserRelationToOrganization.MEMBER_INVITATION_PENDING:
          statusIcon = <MailOutlined style={{ color: "orange" }} />;
          statusTooltip =
            "This user has already been invited to join this Organization";
          break;
      }
      let statusIcon2;
      if (statusIcon) {
        statusIcon2 = <Tooltip title={statusTooltip}>{statusIcon}</Tooltip>;
      }
      const isSelected =
        this.props.selectedUser &&
        user.username === this.props.selectedUser.user.username;
      ...
      return (
        <div
          ...
          key={user.username}
          onClick={() => {
            this.onSelectUser.call(this, user);
          }}
        >
          <div className="InviteUser-statusCol">{statusIcon2}</div>
  ...
  renderInvitationForm() {
    ...
    if (this.props.selectedUser) {
      if (
        this.props.selectedUser.relation ===
        orgModel.UserRelationToOrganization.VIEW
      ) {
        button = (
          <Button type="primary" onClick={this.onSendInvitation.bind(this)}>
            Send Invitation
          </Button>
        );
      } else {
        ...
        message = (
          <div>
            User may not be invited because:
            <br />
    ...
    return (
      <div className="invitationForm">
        <div style={{ textAlign: "center", marginTop: "10px" }}>{button}</div>
  ...
  renderRelation(relation: orgModel.UserRelationToOrganization) {
    switch (relation) {
      ...
      case orgModel.UserRelationToOrganization.VIEW:
        return (
          <span>
            <StopOutlined /> Not a member
          </span>
        );
      case orgModel.UserRelationToOrganization.MEMBER_REQUEST_PENDING:
        return (
          <span>
            <UserOutlined style={{ color: "orange" }} /> User's membership{" "}
      ...
      case orgModel.UserRelationToOrganization.MEMBER_INVITATION_PENDING:
        return (
          <span>
            <UserOutlined style={{ color: "blue" }} /> User has been{" "}
            <b>invited</b> to join
          </span>
        );
      case orgModel.UserRelationToOrganization.MEMBER:
        return (
          <span>
            <UserOutlined /> User is a <b>member</b> of this organization
          </span>
        );
      case orgModel.UserRelationToOrganization.ADMIN:
        return (
          <span>
            <UnlockOutlined />
            User is an <b>admin</b> of this organization
          </span>
        );
      case orgModel.UserRelationToOrganization.OWNER:
        return (
          <span>
            <CrownOutlined /> User is the <b>owner</b> of this Organization
          </span>
        );
    }
  }
    ...
    if (this.props.selectedUser === null) {
      return (
        <div className="InviteUser-selectedUser">
          <p className="InviteUser-noSelection">No user yet selected</p>
        </div>
      );
    } else {
      return (
  ...
  renderFooter() {
    return <div className="InviteUser-footer"></div>;
  }
      ...
      <div className="InviteUser scrollable-flex-column">
        ...
        <div className="InviteUser-row scrollable-flex-row">
          <div className="InviteUser-col1 -firstCol InviteUser-users scrollable-flex-column">
            ...
            <div className="InviteUser-usersTable">{this.renderUsers()}</div>
          </div>
