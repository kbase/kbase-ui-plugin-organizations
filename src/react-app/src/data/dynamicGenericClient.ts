...
export class DynamicServiceClient {
    ...
    constructor({ token, url, module, version, timeout, rpcContext }: DynamicServiceClientParams) {
    ...
    lookupModule() {
        return this.getCached((): Bluebird<any> => {
            ...
            return Bluebird.resolve(client.callFunc('get_service_status', {
                module_name: this.module,
                version: this.version
            }))
    ...
    callFunc(funcName: string, params: any) {
        return this.lookupModule()
            .then(([serviceStatus]) => {
                ...
                return Bluebird.resolve(client.callFunc(funcName, params))
            })
    }
}
