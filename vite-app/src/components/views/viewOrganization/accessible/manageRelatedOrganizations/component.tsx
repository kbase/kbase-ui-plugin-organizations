...
export default class ManageRelatedOrganizations extends Component<
  ManageRelatedOrganizationsProps,
  ManageRelatedOrganizationsState
> {
  onAddSelectedOrganization() {
    ...
    this.props.onAddOrganization(
      ...
      this.props.selectedOrganization.organization.id
    );
  }
  ...
  renderAvailableToolbar() {
    return (
      <div className="ManageRelatedOrganizations-available-toolbar">
        <Input
          allowClear
          placeholder="Filter Available Organizations"
  ...
  renderAvailableOrgs() {
    return this.props.organizations.map(
      (org: SelectableRelatableOrganization) => {
        ...
        let classes = [
          "SimpleCard",
          "ManageRelatedOrganizations-availableOrganization",
        ];
        if (org.isSelected) {
  ...
  renderSelectedOrg() {
    ...
    if (!this.props.selectedOrganization) {
      const message = (
        <Fragment>
          <p>
            When you select an organization on the left, details about it will
            be displayed here.
          </p>
        </Fragment>
      );
      ...
      content = (
        <div className="ManageRelatedOrganizations-selectedOrganization">
          <BriefOrganization
            organization={this.props.selectedOrganization.organization}
            openRequestsStatus={null}
          />
        </div>
      );
      if (this.props.selectedOrganization.isRelated) {
        alert = (
          <Alert
            type="warning"
            message="This org is associated"
            style={alertStyle}
          />
        );
        button = (
    ...
    return (
      <Fragment>
        {alert}
        {content}
        <div className="ManageRelatedOrganizations-buttonBar">{button}</div>
      </Fragment>
    );
  }
      ...
      <div className="ManageRelatedOrganizations scrollable-flex-column">
        <div className="ManageRelatedOrganizations-toolbar">
          <Button onClick={() => this.props.onFinish()} danger>
            <RollbackOutlined /> Done
          </Button>
        </div>
        <div className="ManageRelatedOrganizations-main scrollable-flex-row">
