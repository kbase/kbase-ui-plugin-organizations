...
function groupPermissionToWorkspacePermission(
  groupsPermission: string
...
export function groupToOrganization(
  ...
  const owner: Member = {
    ...
    lastVisitedAt: group.owner.lastvisit
      ? new Date(group.owner.lastvisit)
      : null,
  ...
  const narratives: Array<NarrativeResource> = group.resources.workspace.map(
    (info) => {
      return {
...
function applySort(
  ...
  const direction =
    sortDirection === groupsApi.SortDirection.ASCENDING ? 1 : -1;
  return organizations.slice().sort((a, b) => {
    return (
      applySortComparison(sortField, direction, a, b) ||
      applySortComparison("name", 1, a, b)
    );
  });
}

function groupRoleToUserRelation(
  role: groupsApi.Role
...
export function applyNarrativeSort(
  narratives: Array<NarrativeResource>,
  sortBy: string
) {
  if (!sortBy) {
  ...
  return narratives
    .slice()
    .sort((a: NarrativeResource, b: NarrativeResource) => {
...
export function applyNarrativeSearch(
  narratives: Array<NarrativeResource>,
  searchBy: string
) {
...
export function queryNarratives(
  narratives: Array<NarrativeResource>,
...
export class OrganizationModel {
  ...
  async getOrg(
    id: OrganizationID
  ): Promise<Organization | InaccessiblePrivateOrganization> {
    const group = await this.groupsClient.getGroupById(id);

    // get users from user model (profile)
    if (group.role === "None" && group.private) {
      return groupToPrivateOrganization(
        group as groupsApi.InaccessiblePrivateGroup
      );
    } else {
  ...
  async getOrgs(
    ids: Array<OrganizationID>
  ): Promise<Array<Organization | InaccessiblePrivateOrganization>> {
    return Promise.all(
      ids.map((id) => {
  ...
  async getAllOrgs2(): Promise<Array<BriefOrganization>> {
    ...
    let groups: Array<groupsApi.BriefGroup> =
      await this.groupsClient.listGroups();
  ...
  async queryOrgs(query: Query): Promise<QueryResults> {
    ...
    const filtered = applyFilter(orgs, query.filter);
  ...
  async ownOrgs(): Promise<QueryResults> {
    const orgs = await this.getAllOrgs2();
    const ownOrgs = orgs.filter((org: BriefOrganization) => {
      return org.relation !== UserRelationToOrganization.NONE;
    });
  ...
  async getOpenRequestsStatus({
    ...
    const openRequests = await groupsClient.getOpenRequests({
  ...
  async addRelatedOrganization({
    ...
    return await groupsClient.addRelatedGroup(
      organizationId,
      relatedOrganizationId
    );
  }
  ...
  async removeRelatedOrganization({
    ...
    return await groupsClient.removeRelatedGroup(
      organizationId,
      relatedOrganizationId
    );
  }
...
export function userPermissionToWorkspacePermission(
