...
export function promoteToAdminSuccess(
  state: ViewMembersViewModel,
  action: actions.PromoteToAdminSuccess,
): ViewMembersViewModel {
...
export function demoteToMemberSuccess(
  state: ViewMembersViewModel,
  action: actions.DemoteToMemberSuccess,
): ViewMembersViewModel {
  if (state.loadingState !== AsyncModelState.SUCCESS) {
    return state;
  }
...
function localReducer(
  ...
  switch (action.type) {
    ...
    case ActionFlag.VIEW_MEMBERS_PROMOTE_TO_ADMIN_SUCCESS:
      return promoteToAdminSuccess(
        state,
        action as actions.PromoteToAdminSuccess,
      );
    case ActionFlag.VIEW_MEMBERS_DEMOTE_TO_MEMBER_SUCCESS:
      return demoteToMemberSuccess(
        state,
        action as actions.DemoteToMemberSuccess,
      );
    default:
      return null;
  }
