...
import { StoreState, RequestResourceType } from "../../store/types";
...
export function inviteUserSearchUsers(query: UserQuery) {
  return async (
    ...
    try {
      ...
      const orgUsers: Array<OrganizationUser> = users.map(
        ({ username, realname }) => {
          ...
          if (username === owner) {
            ...
            relation =
              orgModel.UserRelationToOrganization.MEMBER_REQUEST_PENDING;
...
function getUserRelation(
...
): orgModel.UserRelationToOrganization {
  ...
  if (
    inboxRequests.some((request) => {
      return (
        request.resourceType === RequestResourceType.USER &&
...
export function sendInvitation() {
  return (
    ...
    requestClient
      .inviteUserToJoinOrg(id, selectedUser.user.username)
      .then(() => {
